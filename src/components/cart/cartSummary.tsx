...
export const CartSummary = ({ onCheckout }: CartSummaryProps) => {
  ...
  const canCheckout =
    items.length > 0 && isMinimumCheckoutValueReached(totals.subtotal);

  const remainingAmount = Math.max(minimumCheckoutValue - totals.subtotal, 0);
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-5 space-y-3">
        ...
        <div className="flex justify-between text-sm">
          <span className="text-slate-600">Discount (10% above $100)</span>
      ...
      {!canCheckout && items.length > 0 && (
        <div className="mt-5 rounded-lg border border-amber-200 bg-amber-50 p-4">
          ...
          <p className="mt-1 text-sm text-amber-800">
            Add <strong>{formatCurrency(remainingAmount)}</strong> more to your
      ...
      >
        {canCheckout ? "Continue to shipping" : "Checkout unavailable"}
      </button>
    </div>
  );
};
